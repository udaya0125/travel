                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5 md:gap-6">
                    {loading ? (
                        ...
                        allDestinations.map((destination) => (
                            <div
                                key={destination.id}
                                className="group relative overflow-hidden rounded-2xl h-[300px] xs:h-[340px] sm:h-[360px] md:h-[400px]"
                            >
                                {/* Image */}
                                {destination.images?.[0]?.image ? (
                                    <img
                                        src={`${imgurl}/${destination.images[0].image}`}
                                        alt={destination.title}
                                        className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-110"
                                    />
                                ) : (
                                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-gradient-to-br from-[#D8D0C6] to-[#C5BCB0]">
                                        <ImageOff size={28} className="text-[#3A2D26]/50" />
                                        <span className="text-xs font-medium text-[#3A2D26]/60">
                                            No image available
                                        </span>
                                    </div>
                                )}
                                ...
                                {/* Rating pill */}
                                <div className="absolute top-4 right-14 z-10 flex items-center gap-1 bg-white/90 backdrop-blur text-[#3A2D26] text-xs font-medium px-3 py-1 rounded-full">
                                    <Star size={12} className="text-[#79824E]" fill="currentColor" />
                                    {destination.rating}
                                </div>

                                {/* Admin actions — always visible, top-right */}
                                <div className="absolute top-4 right-4 z-20 flex flex-col items-center gap-2">
                                    <button
                                        onClick={() => handleEdit(destination)}
                                        aria-label="Edit destination"
                                        className="flex items-center justify-center w-8 h-8 bg-white text-[#3A2D26] rounded-full shadow-sm hover:bg-[#79824E] hover:text-white transition-colors duration-200"
                                    >
                                        <Pencil size={14} />
                                    </button>
                                    <button
                                        ...
                                        className="flex items-center justify-center w-8 h-8 bg-white text-red-600 rounded-full shadow-sm hover:bg-red-600 hover:text-white transition-colors duration-200"
                                    >
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 z-10">
                                    ...
                                    <div className="text-white/90 text-xs sm:text-sm leading-relaxed line-clamp-2 [&_p]:m-0 [&_a]:underline">
